...
- Evidence notes: S07 gate passed after independent triple-review consensus. All 17 direct commands parse deterministically with correct validation. Command execution routing contract (`CommandExecutor` type) established. Conversational vs auto mode semantics complete with new-ready-only auto dispatch gating. Note: command execution is contract-level (typed interfaces + parser); full routing/handlers for `scout`, `implement`, etc. through the control API and CLI are follow-up work outside this slice's contract-seed scope.
